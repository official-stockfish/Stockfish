...
static_assert(sizeof(ClusterTT) == 32, "Suboptimal Cluster size");
...
void TranspositionTable::resize(size_t mbSize, ThreadPool& threads) {
    ...
    clusterCount = mbSize * 1024 * 1024 / sizeof(ClusterTT);

    table = static_cast<ClusterTT*>(aligned_large_pages_alloc(clusterCount * sizeof(ClusterTT)));
...
void TranspositionTable::clear(ThreadPool& threads) {
    ...
    for (size_t i = 0; i < threadCount; ++i)
    {
        threads.run_on_thread(i, [this, i, threadCount]() {
            ...
            std::memset(&table[start], 0, len * sizeof(ClusterTT));
        });
    }
