  ...
  if ((((Movesplayed <= 40) || (piecesCnt <= 6)) && (PrvDepth > 4 * ONE_PLY)) && mainThread)
  {
	  ...
	  if (startpoint &&  piecesCnt > 6)
	  {
		  for (int x = 0; x < openingswritten; x++)
		  {
			  string openings;
			  char *opnings;
			  openings = NumberToString(OpFileKey[x]) + ".bin";
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning, int kellyNode) {
        ...
        &&  pos.non_pawn_material(pos.side_to_move()))
    {
        ...
        Value nullValue = depth-R < ONE_PLY ? -qsearch<NonPV, false>(pos, ss+1, -beta, -beta+1)
			: -search<NonPV>(pos, ss + 1, -beta, -beta + 1, depth - R, !cutNode, true, kellyNode);
        pos.undo_null_move();
        ...
        if (nullValue >= beta)
        {
            ...
            Value v = depth-R < ONE_PLY ? qsearch<NonPV, false>(pos, ss, beta-1, beta)
				: search<NonPV>(pos, ss, beta - 1, beta, depth - R, false, true, kellyNode);
        ...
        &&  abs(beta) < VALUE_MATE_IN_MAX_PLY)
    {
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            if (pos.legal(move))
            {
                ...
                pos.do_move(move, st);
				value = -search<NonPV>(pos, ss + 1, -rbeta, -rbeta + 1, rdepth, !cutNode, false, kellyNode);
                pos.undo_move(move);
                if (value >= rbeta)
                    return value;
            }
        ...
        && (PvNode || ss->staticEval + 256 >= beta))
    {
		...
		search<NT>(pos, ss, alpha, beta, d, cutNode, true, kellyNode);
