...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (((PvNode || cutNode) && depth >= (PvNode? 5: 7)) && !ttData.move)
        depth--;
...
moves_loop:  // When in check, search starts here
    ...
    {
        int bonusScale =
          (std::min(73 * depth - 347, 184) + 33 * !allNode + 174 * ((ss - 1)->moveCount > 8)
           + 144 * (!ss->inCheck && bestValue <= ss->staticEval - 104)
           + 128 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 82)
           + 86 * ((ss - 1)->isTTMove) + 90 * (ss->cutoffCnt <= 3)
           + std::min(-(ss - 1)->statScore / 113, 293));
