...
namespace Stockfish {
    ...
    namespace {
        ...
        void update_all_stats(const Position &pos, Stack *ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
                              Move *quietsSearched, int quietCount, Move *capturesSearched, int captureCount,
                              Depth depth);
...
/// consumed, the user stops the search, or the maximum search depth is reached.
    ...
    void Thread::search() {
        ...
        if (skill.enabled())
            multiPV = std::max(multiPV, (size_t)
        4);
    ...
    namespace {
        ...
        Value search(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth, bool cutNode) {
            ...
            improvement = (ss - 2)->staticEval != VALUE_NONE ? ss->staticEval - (ss - 2)->staticEval
                                                             : (ss - 4)->staticEval != VALUE_NONE ? ss->staticEval -
                                                                                                    (ss - 4)->staticEval
                                                                                                  : 168;
            ...
            while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE) {
                ...
                if (!rootNode
                    ...
                    int lmrDepth = std::max(
                            newDepth - reduction(improving, depth, moveCount, delta, thisThread->rootDelta), 0);
...
/// to detect when we are out of available time and thus stop the search.
    ...
    void MainThread::check_time() {
        ...
        if ((Limits.use_time_management() && (elapsed > Time.maximum() - 10 || stopOnPonderhit))
            ...
            || (Limits.nodes && Threads.nodes_searched() >= (uint64_t)
            Limits.nodes))
        Threads.stop = true;
    }
