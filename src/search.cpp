...
void Thread::search() {
  ...
  {
      // Distribute search depths across the threads, picking the next depth to search
      // for each thread in a way to make each thread searching on a different depth.
      if (idx)
      {
          int i = (idx - 1) % 20;
