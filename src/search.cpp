...
#ifndef Maverick  //vondele Randomize draw eval
...
void MainThread::search() {
  ...
  {
      std::map<Move, int64_t> votes; // New voting formula #1876. no fucntional change
      ...
      for (Thread* th : Threads)
          ...
          votes[th->rootMoves[0].pv[0]] += 200 + (square(th->rootMoves[0].score - minScore + 1)
                                                  * int64_t(th->completedDepth));//// New voting formula #1876. no fucntional change
#endif
      // Select best thread
      int64_t bestVote = votes[this->rootMoves[0].pv[0]];//// New voting formula #1876. no fucntional change
      for (Thread* th : Threads)
      {
...
void Thread::search() {
#ifdef Maverick //Extend stack to ss-5, and remove conditions #1887
...
#ifdef Maverick	
  bool w_defensive = Options["White Defensive"];
  bool b_defensive = Options["Black Defensive"];
  int def_focus = Options["Defensive Focus"];
#endif
  ...
  if (Limits.infinite || Options["UCI_AnalyseMode"])
	  ...
	  ct = (w_defensive && us == WHITE ) ? -ct * def_focus /2 : ct ;
	  ct = (b_defensive && us == BLACK ) ? -ct * def_focus /2 : ct ;
  }
#endif
	...
	else if ct = (b_defensive && us == !WHITE ) ? -ct*4/5 : ct ;
	//else if ct = (w_defensive && us ==  WHITE ) ? -ct*4/5 : ct ;*/
         ...
         && !(Limits.depth && mainThread && rootDepth / ONE_PLY > Limits.depth))
  {
      ...
      if (   Limits.mate
        ...
        {
#ifndef Maverick  //Simplify time manager in search() #1819
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && pos.has_game_cycle(ss->ply))
    {
        ...
        alpha = VALUE_DRAW;
#else
        alpha = value_draw(depth, pos.this_thread());  //vondele Randomize draw eval
#endif
        if (alpha >= beta)
            return alpha;
    }
    ...
    if (!rootNode)
    {
        ...
        if (   Threads.stop.load(std::memory_order_relaxed)
                || pos.is_draw(ss->ply)
                || ss->ply >= MAX_PLY)
#ifdef Maverick //vondele Randomize draw eval
            return (ss->ply >= MAX_PLY && !inCheck) ? evaluate(pos) : VALUE_DRAW;
#else
            return (ss->ply >= MAX_PLY && !inCheck) ? evaluate(pos)    // vondele Randomize draw eval
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
          ...
          &&  pos.legal(move))
      {
        ...
        if (   pos.promotion_pawn_push(move) && !moveCountPruning && pos.non_pawn_material(us)> BishopValueEg * 2 )
            extension = ONE_PLY;
