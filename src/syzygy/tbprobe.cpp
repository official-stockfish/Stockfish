    ...
    public:
        ...
        uint8_t *map(void **baseAddress, uint64_t *mapping, TBType type) {
            ...
            *baseAddress = MapViewOfFile(mmap, FILE_MAP_READ, 0, 0, 0);
            ...
            constexpr
            uint8_t Magics[][4] = {{0xD7, 0x66, 0x0C, 0xA5},
                                   {0x71, 0xE8, 0x23, 0x5D}};
...
// file a,b,c,d also in this case one set for wtm and one for btm.
    int decompress_pairs(PairsData *d, uint64_t idx) {
        ...
        uint32_t * ptr = (uint32_t * )(d->data + ((uint64_t)
        block * d->sizeofBlock));
        ...
        while (true) {
            ...
            if (buf64Size <= 32) { // Refill the buffer
                buf64Size += 32;
                buf64 |= (uint64_t)
                number<uint32_t, BigEndian>(ptr++) << (64 - buf64Size);
            }
        }
    ...
    for (auto &m: rootMoves) {
        ...
        m.tbScore = r >= bound ? VALUE_MATE - MAX_PLY - 1
                               : r > 0 ? Value((std::max(3, r - (MAX_DTZ - 200)) * int(PawnValueEg)) / 200)
                                       : r == 0 ? VALUE_DRAW
                                                : r > -bound ? Value(
                                        (std::min(-3, r + (MAX_DTZ - 200)) * int(PawnValueEg)) / 200)
                                                             : -VALUE_MATE + MAX_PLY + 1;
    }
