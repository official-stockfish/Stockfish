...
Bitboard RanksForwardBB[COLOR_NB][RANK_NB];
Bitboard BetweenBB[SQUARE_NB][SQUARE_NB];
Bitboard LineBB[SQUARE_NB][SQUARE_NB];
Bitboard DistanceRingBB[SQUARE_NB][8];
Bitboard FileForwardBB[COLOR_NB][SQUARE_NB];
...
void Bitboards::init() {
  ...
  for (Rank r = RANK_1; r < RANK_8; ++r)
      RanksForwardBB[WHITE][r] = ~(RanksForwardBB[BLACK][r + 1] = RanksForwardBB[BLACK][r] | RankBB[r]);

  for (Color c = WHITE; c <= BLACK; ++c)
      for (Square s = SQ_A1; s <= SQ_H8; ++s)
      {
          FileForwardBB [c][s] = RanksForwardBB[c][rank_of(s)] & FileBB[file_of(s)];
          PawnAttackSpan[c][s] = RanksForwardBB[c][rank_of(s)] & AdjacentFilesBB[file_of(s)];
          PassedPawnMask[c][s] = FileForwardBB [c][s] | PawnAttackSpan[c][s];
      }
