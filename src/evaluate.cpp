...
namespace {
  ...
  void eval_init(const Position& pos, EvalInfo& ei) {
    ...
    // Find our pawns on the 2nd or 3rd rank, and those which are blocked
    Bitboard b = pos.pieces(Us, PAWN) & (shift<Down>(pos.pieces()) | LowRanks);

    // Squares occupied by those pawns, by our king, or controlled by enemy pawns,
    // are excluded from the mobility area.
    ei.mobilityArea[Us] = ~(b | pos.square<KING>(Us) | ei.pi->pawn_attacks(Them));
    ...
    if (pos.non_pawn_material(Them) >= QueenValueMg)
    {
        ...
        ei.kingAttackersCount[Them] = popcount(b & ei.pi->pawn_attacks(Them));
