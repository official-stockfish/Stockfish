...
Value Endgame<KXK>::operator()(const Position& pos) const {
  ...
  Value result =  pos.non_pawn_material(strongSide)
                ...
                + PushClose[dist(winnerKSq, loserKSq)];
...
Value Endgame<KRKP>::operator()(const Position& pos) const {
  ...
  if (wksq < psq && file_of(wksq) == file_of(psq))
      result = RookValueEg - dist(wksq, psq);

  // If the weaker side's king is too far from the pawn and the rook,
  // it's a win.
  else if (   dist(bksq, psq) >= 3 + (pos.side_to_move() == weakSide)
           && dist(bksq, rsq) >= 3)
      result = RookValueEg - dist(wksq, psq);
  ...
  // the position is drawish
           ...
           && dist(bksq, psq) == 1
           && rank_of(wksq) >= RANK_4
           && dist(wksq, psq) > 2 + (pos.side_to_move() == strongSide))
      result = Value(80) - 8 * dist(wksq, psq);

  else
      result =  Value(200) - 8 * (  dist(wksq, psq + DELTA_S)
                                  - dist(bksq, psq + DELTA_S)
                                  - dist(psq, queeningSq));
...
Value Endgame<KRKN>::operator()(const Position& pos) const {
  ...
  Value result = Value(PushToEdges[bksq] + PushAway[dist(bksq, bnsq)]);
  return strongSide == pos.side_to_move() ? result : -result;
}
...
Value Endgame<KQKP>::operator()(const Position& pos) const {
  ...
  Value result = Value(PushClose[dist(winnerKSq, loserKSq)]);

  if (   relative_rank(weakSide, pawnSq) != RANK_7
      || dist(loserKSq, pawnSq) != 1
...
ScaleFactor Endgame<KBPsK>::operator()(const Position& pos) const {
      ...
      && pos.count<PAWN>(weakSide) >= 1)
  {
      ...
      {
          int strongKingDist = dist(weakPawnSq, strongKingSq);
          int weakKingDist = dist(weakPawnSq, weakKingSq);
...
ScaleFactor Endgame<KRPKR>::operator()(const Position& pos) const {
  ...
  if (   r == RANK_7
      ...
      && file_of(wrsq) == f
      && wrsq != queeningSq
      && (dist(wksq, queeningSq) < dist(bksq, queeningSq) - 2 + tempo)
      && (dist(wksq, queeningSq) < dist(bksq, wrsq) + tempo))
      return ScaleFactor(SCALE_FACTOR_MAX - 2 * dist(wksq, queeningSq));
  ...
  if (   f != FILE_A
      ...
      && wrsq < wpsq
      && (dist(wksq, queeningSq) < dist(bksq, queeningSq) - 2 + tempo)
      && (dist(wksq, wpsq + DELTA_N) < dist(bksq, wpsq + DELTA_N) - 2 + tempo)
      && (  dist(bksq, wrsq) + tempo >= 3
          || (    dist(wksq, queeningSq) < dist(bksq, wrsq) + tempo
              && (dist(wksq, wpsq + DELTA_N) < dist(bksq, wrsq) + tempo))))
      return ScaleFactor(  SCALE_FACTOR_MAX
                         - 8 * dist(wpsq, queeningSq)
                         - 2 * dist(wksq, queeningSq));
  ...
  if (r <= RANK_4 && bksq > wpsq)
  {
      ...
      if (   dist(file_of(bksq), file_of(wpsq)) == 1
          && dist(wksq, bksq) > 2)
          return ScaleFactor(24 - 2 * dist(wksq, bksq));
  }
  return SCALE_FACTOR_NONE;
}
...
ScaleFactor Endgame<KNPK>::operator()(const Position& pos) const {
  ...
  if (pawnSq == SQ_A7 && dist(SQ_A8, weakKingSq) <= 1)
      return SCALE_FACTOR_DRAW;
...
ScaleFactor Endgame<KNPKB>::operator()(const Position& pos) const {
  ...
  if (forward_bb(strongSide, pawnSq) & pos.attacks_from<BISHOP>(bishopSq))
      return ScaleFactor(dist(weakKingSq, pawnSq));
