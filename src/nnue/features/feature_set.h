  ...
  class FeatureSetBase {
   ...
   public:
    ...
    static void AppendChangedIndices(
      ...
      auto collect_indices = [&](const DirtyPiece* dp1, const DirtyPiece* dp2) {
        for (Color perspective : { WHITE, BLACK }) {
          switch (trigger) {
            case TriggerEvent::kFriendKingMoved:
              reset[perspective] = dp1->piece[0] == make_piece(perspective, KING)
                                || (dp2 && dp2->piece[0] == make_piece(perspective, KING));
              break;
            default:
              assert(false);
              break;
      ...
      if (pos.state()->previous->accumulator.computed_accumulation) {
        ...
        collect_indices(&prev_dp, nullptr);
      } else {
        ...
        if (prev_dp.dirty_num == 0) {
          ...
          collect_indices(&prev2_dp, nullptr);
        } else {
          const auto& prev2_dp = pos.state()->dirtyPiece;
          if (prev2_dp.dirty_num == 0) {
            collect_indices(&prev_dp, nullptr);
          } else {
            collect_indices(&prev_dp, &prev2_dp);
          }
        }
      }
    }
