...
inline void huffman_build_lengths(const std::vector<u64>& freq, std::vector<u8>& lengths) {
    ...
    // Unconstrained code lengths come from the depths of a Huffman tree, of
    // which only the link from each node to its parent needs to be kept
    std::vector<i32> link(2 * used.size() - 1);

    using Entry = std::pair<u64, i32>;  // (frequency, node index)
    std::priority_queue<Entry, std::vector<Entry>, std::greater<Entry>> queue;

    for (usize i = 0; i < used.size(); ++i)
        queue.emplace(freq[used[i]], i32(i));

    for (usize next = used.size(); queue.size() > 1; ++next)
    {
        ...
        link[a.second] = link[b.second] = i32(next);
        queue.emplace(a.first + b.first, i32(next));
    }

    // A parent is always merged after its children, so a single backward pass
    // turns the links into depths, the root sitting at depth zero
    link.back() = 0;
    for (i32 i = i32(link.size()) - 2; i >= 0; --i)
        link[i] = link[link[i]] + 1;

    // Count how many codes end up at each depth
    std::array<u32, HuffmanMaxRawLength + 2> counts{};
    for (usize i = 0; i < used.size(); ++i)
    {
        assert(link[i] >= 1 && link[i] <= i32(HuffmanMaxRawLength));
        ++counts[link[i]];
    }
...
// Visits symbols in canonical-code order
template<typename Func>
inline void huffman_for_each_code(const std::vector<u8>& lengths, Func&& func) {
...
inline void write_huffman_block(std::ostream& stream, const IntType* values, usize count) {
    ...
    const auto [minIt, maxIt] = std::minmax_element(values, values + count);
    const i64 lo = *minIt, hi = *maxIt;
    ...
    std::vector<u32> codes(alphabet);
    u64              bits = 0;
    huffman_for_each_code(lengths, [&](usize s, u32 code, u32 length) {
        codes[s] = code;
        bits += u64(length) * freq[s];
    });
...
read_huffman_block(std::istream& stream, IntType* out, usize count, std::vector<u32>& table) {
    ...
    const u32 tableBits = std::max<u32>(1, *std::max_element(lengths.begin(), lengths.end()));

    huffman_for_each_code(lengths, [&](usize s, u32 code, u32 length) {
        const u32 shift = tableBits - length;
        std::fill_n(table.begin() + (code << shift), usize(1) << shift,
