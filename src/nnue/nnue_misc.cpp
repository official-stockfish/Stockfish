...
constexpr std::array<char, 13> PieceToChar = {' ', 'P', 'N', 'B', 'R', 'Q', 'K', ' ', 'p', 'n', 'b', 'r', 'q', 'k'};
...
void format_cp_compact(Value v, char* buffer, const Position& pos) {
    ...
    if (cp >= 10000) {
        std::snprintf(buffer + 1, 4, "%04d", cp / 100);
        buffer[4] = ' ';
    } else if (cp >= 1000) {
        std::snprintf(buffer + 1, 5, "%d.%02d", cp / 100, cp % 100);
    } else {
        std::snprintf(buffer + 1, 5, "%d.%02d", cp / 100, cp % 100);
    }
}
...
std::string trace(Position& pos, const Eval::NNUE::Networks& networks, Eval::NNUE::AccumulatorCaches& caches) {
    std::stringstream ss;
    std::array<std::array<char, 8 * 8 + 2>, 3 * 8 + 1> board{};
