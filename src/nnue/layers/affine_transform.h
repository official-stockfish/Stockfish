...
// Identical for both approaches. Requires the input to be padded to at least 16 values.
    ...
    static void affine_transform_non_ssse3(std::int32_t *output, const std::int8_t *weights, const std::int32_t *biases,
...
#else
    constexpr IndexType
    LargeInputSize = std::numeric_limits<IndexType>::max();
#endif

    // A specialization for large inputs.
    template<IndexType InDims, IndexType OutDims>
    class AffineTransform<InDims, OutDims,
...

{
    ...
    static_assert(PaddedInputDimensions
    >= LargeInputSize, "Something went wrong. This specialization should not have been chosen.");
    ...
    static constexpr const IndexType
    InputSimdWidth = 1;
    static constexpr const IndexType
    MaxNumOutputRegs = 1;
#endif
    ...
    static constexpr const IndexType
    NumOutputRegs = std::min(MaxNumOutputRegs, OutputDimensions);
    static constexpr const IndexType
    SmallBlockSize = InputSimdWidth;
    static constexpr const IndexType
    BigBlockSize = NumOutputRegs * PaddedInputDimensions;
    static constexpr const IndexType
    NumSmallBlocksInBigBlock = BigBlockSize / SmallBlockSize;
    static constexpr const IndexType
    NumSmallBlocksPerOutput = PaddedInputDimensions / SmallBlockSize;
    static constexpr const IndexType
    NumBigBlocks = OutputDimensions / NumOutputRegs;

    static_assert(OutputDimensions
    % NumOutputRegs == 0);

    // Hash value embedded in the evaluation file
    static constexpr std::uint32_t
    get_hash_value(std::uint32_t
    prevHash) {
...
static_assert(PaddedInputDimensions<LargeInputSize,
"Something went wrong. This specialization should not have been chosen.");
...
static IndexType get_weight_index_scrambled(IndexType
i)
{
return
...
static IndexType get_weight_index(IndexType
i)
{
#if defined (USE_SSSE3)
