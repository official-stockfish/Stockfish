...
*/
  ...
  while ((ss >> token) && !isspace(token))
  {
      ...
      {
          put_piece(color_of(Piece(idx)), Piece(idx), sq);
          ++sq;
      }
  }
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  if (type_of(m) == CASTLING)
  {
      ...
      do_castling<true>(us, pc, rook, from, to, rfrom, rto);
...
void Position::undo_move(Move m) {
  ...
  {
      Square rfrom, rto;
      do_castling<false>(us, make_piece(us, KING), make_piece(us, ROOK), from, to, rfrom, rto);
  }
  else
  {
      move_piece(us, pc, to, from); // Put the piece back at the source square
      ...
      if (st->capturedPiece)
      {
          ...
          put_piece(~us, st->capturedPiece, capsq); // Restore the captured piece
      }
  }
...
template<bool Do>
void Position::do_castling(Color us, Piece king, Piece rook, Square from, Square& to, Square& rfrom, Square& rto) {
  ...
  for (int step = Default; step <= (Fast ? Default : Castling); step++)
  {
      ...
      if (step == Lists)
          for (Color c = WHITE; c <= BLACK; ++c)
              for (PieceType pt = PAWN; pt <= KING; ++pt)
              {
                  if (pieceCount[make_piece(c, pt)] != popcount(pieces(c, pt)))
                      return false;

                  for (int i = 0; i < pieceCount[make_piece(c, pt)];  ++i)
                      if (   board[pieceList[make_piece(c, pt)][i]] != make_piece(c, pt)
                          || index[pieceList[make_piece(c, pt)][i]] != i)
                          return false;
              }
