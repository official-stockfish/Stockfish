...
[StockfishNPS](https://github.com/Joachim26/Stockfish) is a slightly modified Stockfish with nps reduction, randomized evaluation, and search-depth limit. The strength of the first two modifications can be set in the uci options from 0 to 100, while the search-depth may be limited to values from 1 to 20.
All three mods can be used in any combination together with UCI_Elo and Skill Level settings for human play. Without these settings, StockfishNPS plays exactly like the current Stockfish15_dev.
