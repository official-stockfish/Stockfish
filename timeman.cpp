...
void TimeManagement::init(Search::LimitsType& limits,
 ...
 double phaseFactor = 1.0;
        if (ply < 12) {
            phaseFactor = 0.4 + 0.05 * ply; 
        }
        optScale *= phaseFactor;
    }
    ...
    maximumTime =
      TimePoint(std::min(0.825179 * limits.time[us] - moveOverhead, maxScale * optimumTime)) - 10;
       ...
       if (limits.movestogo == 0 && ply < 10) {
        TimePoint maxEarlyTime = TimePoint(0.3 * limits.time[us]);
        optimumTime = std::min(optimumTime, maxEarlyTime);
        maximumTime = std::max(maximumTime, optimumTime + 10);
       }
    if (options["Ponder"])
